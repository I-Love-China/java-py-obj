 ...
 * Java 对象转换访问者
 * 
 * 实现访问者模式的具体访问者，负责将 PythonValue 对象转换为 Java 原生对象。
 * 这个访问者提供了直接的Java对象转换，跳过JSON序列化步骤，适合需要在
 * Java 代码中直接操作数据的场景。
 * 
 * 设计理念：
 * - 直接转换：避免 JSON 序列化/反序列化的开销
 * - 类型保持：尽可能保持原始的 Java 类型信息
 * - 性能优化：使用最适合的 Java 数据结构
 * - 易于扩展：支持多种输出格式的统一处理
 * 
 * 转换映射规则：
 * ┌─────────────────┬──────────────────┬──────────────────────────────┐
 * │ Python 类型      │ PythonValue 类型  │ Java 类型                     │
 * ├─────────────────┼──────────────────┼──────────────────────────────┤
 ...
 * - 字符串：保持 UTF-8 编码
 * - 容器类型：使用 Object[] 数组保持顺序
 * - 字典：使用 HashMap<String, Object> 提供高效查找
 * 
 * 使用场景：
 * - 需要在 Java 中直接处理数据
 * - 避免 JSON 字符串解析开销
 ...
 * 性能特征：
 * - 时间复杂度：O(n)，n为节点总数
 * - 空间复杂度：O(n)，创建对应的 Java 对象
 * - 内存效率：比 JSON 字符串更节省内存
 * 
 * 线程安全性：
 * - 访问者本身是无状态的，线程安全
...
public class JavaObjectVisitor implements PythonValueVisitor<Object> {
    ...
    /**
     * 访问基本类型值，直接返回存储的 Java 对象
     * 
     * 基本类型值在词法分析阶段已经转换为对应的 Java 对象，
    ...
    /**
     * 访问列表类型值，转换为 Java 对象数组
     * 
     * 将 Python 列表转换为 Java Object[] 数组：
     * 1. 获取列表中的所有元素
     * 2. 递归转换每个元素为 Java 对象
     * 3. 收集到 Object[] 数组中
     * 4. 保持元素顺序
    ...
    /**
     * 访问字典类型值，转换为 Java Map
     * 
     * 将 Python 字典转换为 Java HashMap<String, Object>：
     * 1. 创建新的HashMap
     * 2. 遍历字典中的每个键值对
     * 3. 将键转换为字符串（确保 Map 键的统一性）
     * 4. 递归转换值对象
     * 5. 存入 HashMap 中
     * 
     * HashMap 选择理由：
     ...
     * 键转换策略：
     * - 先递归转换为Java对象
     * - 再使用 String.valueOf 转换为字符串
     * - 处理各种键类型（数字、字符串、布尔值等）
     * 
     * 潜在问题：
     * - 键冲突：不同类型的键可能转换为相同字符串
     * - 解决方案：依赖 Python 源码的正确性
     * 
    ...
    /**
     * 访问元组类型值，转换为 Java 对象数组
     * 
     * 元组转换策略与列表相同，因为Java中没有专门的元组类型，
     * 统一使用 Object[] 数组表示有序序列。
     * 
     * 设计考虑：
     * - 保持元组的不可变语义（虽然 Object[] 本身可变）
     * - 保持元素顺序
     * - 支持异构元素类型
     * - 与 JSON 数组保持一致
     * 
    ...
    /**
     * 访问集合类型值，转换为 Java 对象数组
     * 
     * 集合转换策略与列表相同，因为 Java 中的 Set 类型在序列化时
     * 通常也表示为数组形式，为保持一致性使用 Object[] 数组。
     * 
     * 设计权衡：
     * - 不使用 Java Set<Object>：因为需要与 JSON 数组保持一致
     * - 不进行去重处理：信任 Python 源码的正确性
     * - 不保证元素顺序：集合本身是无序的
     * 
     * 替代方案考虑：
     * - 可以返回 HashSet<Object> 保持集合语义
